...
const Hyperspeed = ({
  ...
  useEffect(() => {
    // Only dispose if we're unmounting or if appRef.current exists
    const cleanup = () => {
      if (appRef.current) {
  ...
  }, [effectOptions.distortion]); // Only depend on distortion type to prevent unnecessary recreations
